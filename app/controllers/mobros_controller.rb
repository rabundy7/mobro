class MobrosController < ApplicationController
  ...
  def add_column
    ...
    # Run migration
    %x[rake db:migrate]
    render :template => 'mobros/add_column.js.rjs', :locals => {:model => Object.const_get(params[:model].titleize)}
  end
  ...
  def add_model
    ...
    # Run migration
    %x[rake db:migrate]
    render :template => 'mobros/add_model.js.rjs', :locals => {:model => Object.const_get(params[:model_name].titleize)}
  end
